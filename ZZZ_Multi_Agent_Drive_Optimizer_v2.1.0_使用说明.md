# ZZZ Multi-Agent Drive Optimizer v2.1.0 使用说明

本说明适用于 `ZZZ_Multi_Agent_Drive_Optimizer_v2.1.0.exe`。
...
1. 双击 `ZZZ_Multi_Agent_Drive_Optimizer_v2.1.0.exe`。
...
如果 Scanner 明确提示权限不足、无法读取游戏窗口或无法开始扫描，请先关闭配装器，然后右键 `ZZZ_Multi_Agent_Drive_Optimizer_v2.1.0.exe`，选择“以管理员身份运行”，再从页面打开 Scanner。正常能够扫描时不需要管理员权限。
...
如果浏览器或系统阻止关闭通知，仍可在任务管理器中结束 `ZZZ_Multi_Agent_Drive_Optimizer_v2.1.0.exe`。
