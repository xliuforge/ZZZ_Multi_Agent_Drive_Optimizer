...
$versionMatch = [regex]::Match([System.IO.File]::ReadAllText((Join-Path $root 'main.go')), 'const releaseSeries = "([^"]+)"')
if (-not $versionMatch.Success) { throw 'releaseSeries was not found in main.go.' }
$version = $versionMatch.Groups[1].Value
$source = Join-Path $root "ZZZ_Multi_Agent_Drive_Optimizer_v${version}_使用说明.md"
if (-not (Test-Path -LiteralPath $source)) { throw "Manual source was not found: $source" }
...
$document = '<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ZZZ Multi-Agent Drive Optimizer v' + $version + ' 使用说明</title><style>' + $style + '</style></head><body><main>' + $body.ToString() + '</main></body></html>'
[System.IO.File]::WriteAllText($target, $document, [System.Text.UTF8Encoding]::new($false))
Write-Output "HTML: $target"
if ($Pdf) {
    $browser = @(
        "$env:ProgramFiles/Google/Chrome/Application/chrome.exe",
        "${env:ProgramFiles(x86)}/Microsoft/Edge/Application/msedge.exe"
    ) | Where-Object { Test-Path -LiteralPath $_ } | Select-Object -First 1
    if (-not $browser) { throw 'PDF rendering requires Chrome or Edge.' }
    $pdfTarget = Join-Path $root "ZZZ_Multi_Agent_Drive_Optimizer_v${version}_使用说明书.pdf"
    $profile = Join-Path $root '.tmp/manual-browser'
    $arguments = @('--headless', '--disable-gpu', '--no-first-run', '--no-default-browser-check', '--no-pdf-header-footer', "--user-data-dir=`"$profile`"", "--print-to-pdf=`"$pdfTarget`"", "`"$(([uri]$target).AbsoluteUri)`"")
    $process = Start-Process -FilePath $browser -ArgumentList $arguments -WindowStyle Hidden -PassThru
    if (-not $process.WaitForExit(60000)) { throw 'PDF rendering timed out.' }
    if (-not (Test-Path -LiteralPath $pdfTarget) -or (Get-Item -LiteralPath $pdfTarget).Length -lt 1000) { throw 'PDF rendering failed.' }
    Write-Output "PDF: $pdfTarget"
}
