...
库存支持卡片/表格视图、关键词筛选、槽位筛选、套装筛选、属性筛选、角色套装筛选、复制、删除和归属管理。
...
选择角色后，程序会加载内置的 60 级角色基础数据。核心技等级会影响基础攻击/生命/防御或角色高级属性。选择音擎后：
...
填写两个或以上目标时，如果用户没有手动选择其他策略，界面会自动切换到“遵守严格指标”。
...
“词条差距”只用于把不同属性换算到可比较尺度，不代表高出目标一定是坏属性；严格指标策略的目标是尽可能贴近期望值。
...
- 修改后必须点击“保存 / 更新当前角色目标”。
- **重新计算此角色**：只重新计算该角色，不重新跑所有角色。

单角色重新计算会锁定排序在它前面的角色所用驱动盘。低优先级角色不会反过来限制它；如果新方案占用了后续角色的盘，只清除发生冲突的后续结果，其他角色结果保留。
...
请使用最新 EXE 重新计算。新版本会把角色基础冲击力传给后端，并自动兼容旧多角色目标。
