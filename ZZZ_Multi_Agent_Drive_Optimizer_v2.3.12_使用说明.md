# ZZZ Multi-Agent Drive Optimizer v2.3.12 使用说明

本说明适用于 `ZZZ_Multi_Agent_Drive_Optimizer_v2.3.12.exe`。

更新时间：2026-09-19。`v2.3.12`代表工具第2代、绝区零3.1版本第2位新角色更新（希格莉德），是本项目的自定义版号规则。

本次新增希格莉德及骁骑礼赞，支持核心0～F、音擎1～5阶、单角色与多角色配装，并修正触发暴击加成溢出时的排序扣减。更新时将新EXE放在原程序目录，沿用现有库存和角色目标文件即可；无需重新导入或清空数据。
...
1. 双击 `ZZZ_Multi_Agent_Drive_Optimizer_v2.3.12.exe`。
...
如果 Scanner 明确提示权限不足、无法读取游戏窗口或无法开始扫描，请先关闭配装器，然后右键 `ZZZ_Multi_Agent_Drive_Optimizer_v2.3.12.exe`，选择“以管理员身份运行”，再从页面打开 Scanner。正常能够扫描时不需要管理员权限。
...
- 在“强攻”中选择希格莉德，按实际培养情况选择核心技等级；专武自动匹配“骁骑礼赞”。
- 60级未升级核心基准：生命7788、攻击863、防御606；核心F常驻增加75基础攻击和14.4%暴击率。
- 巡空枪势已触发时，核心0/A/B/C/D/E/F分别提供33/39/44/50/55/61/66%实战暴击率。该加成参与实战参考指数，不计入面板目标。
- 核心F且没有其他实战暴击加成时，填写面板暴击目标34%，对应触发后100%暴击；无需填写100%的面板目标。
- 骁骑礼赞60级提供713基础攻击及48%面板暴伤；实战按兵锋两层计算，1～5阶额外暴伤为64/73.6/83.2/92.8/102.4%。
- 无视冰抗仅展示；失衡易伤、队伍条件、影画和技能循环不包含在通用指数中。基础生命在不同资料站存在1点取整差异，当前采用7788，待实机截图校准。
- 已保存目标点击“重新计算此角色”或“重新协调全队”时，会按所选核心和音擎刷新数据。
...
九个输入框默认全部为空。空白表示该属性不参与目标筛选；不需要填写 `0`。
...
无法全部满足时，程序会先照顾优先级 1，再依次照顾优先级 2、3、4、5、6。多个属性可以设为相同优先级，程序会综合比较。
...
程序会自动比较不同属性，不需要自己换算。一般会优先避免低于目标，略高于目标可以接受。把多个属性设为相同优先级，表示希望程序综合照顾它们；如果某项最重要，建议给它单独设置更高优先级。

### 4.6 自动排序规则

暴击率、暴击伤害、攻击、生命、防御和异常精通可以选择目标类型：

- **至少达到**：适合一般面板要求，达到目标后即视为合格。
- **收益上限**：适合存在明确收益上限的属性，达到后不再要求继续堆高。
- **尽量接近**：适合希望数值贴近指定目标的情况。

异常掌控、能量回复和冲击力没有驱动盘副词条可供微调，因此固定按“至少达到”处理，但仍可填写目标和设置优先级。
...
暴击率、暴击伤害、攻击力、生命值、防御力和异常精通下方提供“计为有效词条”复选框，默认全部不勾选。异常掌控、能量回复和冲击力没有副词条，不提供有效词条勾选。
...
“有效词条”用来帮助比较驱动盘副属性的质量。程序会自动计算，你只需要勾选该角色真正需要的属性。它只统计驱动盘副词条，不会把主属性、角色基础属性、音擎或套装效果算进去。
...
全队协调需要比较大量组合，角色和库存较多时可能需要等待一会儿。程序会尽量找到兼顾全队且互不冲突的方案；分配模式会自动保存，下次启动仍会保留。
...
异常掌控没有副词条，主要受角色、核心技、音擎、6 号位主属性和套装影响。若更新软件后旧结果没有变化，请点击“重新计算此角色”或“重新协调全队”；不需要删除原来的角色目标。
...
如果浏览器或系统阻止关闭通知，仍可在任务管理器中结束 `ZZZ_Multi_Agent_Drive_Optimizer_v2.3.12.exe`。
...
异常掌控没有副词条，请检查角色核心技等级、音擎、6 号位主属性和 2 件套是否与游戏一致。旧的计算结果不会自动改变，需要点击“重新计算此角色”或“重新协调全队”。
